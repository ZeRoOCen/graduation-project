          ...
          <div class="customer_list_container">
            <ul class="customer_listAndOnline_ul">
              ...
              >
                <a
                  class="customer_a"
                  @click="showChatView(info.cname, info.id)"
                >
...
export default {
  ...
  created() {
    this.getUserName();
    this.getUserAvatar();
    this.getcontactNeedInfo(); //用户通过一个商品界面进入联系客服界面时需要进行处理的方法
  },
  ...
  methods: {
    ...
    //用户通过一个商品界面进入联系客服界面时需要进行处理的方法
    async getcontactNeedInfo() {
      const unameAndGname = window.localStorage.getItem("contactNeedInfo");
      if(unameAndGname){
        const uname = unameAndGname.split(",")[0].substring(0);
        const gname = unameAndGname.split(",")[1].substring(0);
        const resCustomerInfo = await this.$http.idExistJudge({ uname, gname }); //添加唯一ID，如果存在或添加成功，则返回该用户唯一ID，用户名，商品
        this.customerInfo = resCustomerInfo;
      }else{
        const loginUserInfo = window.localStorage.getItem("loginUserInfo");
        const infoObj = JSON.parse(loginUserInfo)
        const uname = infoObj.uname;

        const resCustomerList = await this.$http.getCustomerInfo({uname}) //获取客服列表信息
        this.customerInfo = resCustomerList
      }
    },
    //通过想要聊天的对象打开相应的聊天框
    async showChatView(cname, connectid) {
      clearInterval(this.messageTimer); //当切换聊天对象时，关闭定时器
      
      this.toName = cname; //把想进行聊天的客服名赋给toName
